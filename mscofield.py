...
async def logUserBot():
    load_dotenv()
    api_id = int(21481944)
    api_hash = "5d1e1d03768f66777da121a160a3db73"
    phone_number = "51992079975"
    ...
    await client.send_message("@SpamHB15", f'<b>Bot encendido</b>', parse_mode="HTML")
    spammer_group = int("-4254598126")
    ...
    while True:
        ...
        try:
            await client.send_message("@SpamHB15", f"<b>CANTIDAD DE MENSAJES CONSEGUIDOS PARA PUBLICAR</b> <code>{len(messages_list)}</code>",parse_mode="HTML")
        except:
            pass
                ...
                if i['group_name'] not in ["SPAM 2024","RESPALDO🇵🇪BINS PERU","➳𝒀𝑨𝑷𝑬 𝑫𝑬 𝑬𝑺𝑻𝑨𝑭𝑨𝑫𝑶𝑹𝑬𝑺 ✧","QUEMANDO ESTAFADORES","𝐏𝐄𝐑Ú 𝐀𝐘𝐔𝐃𝐀","Referencias Elmer 💸","🎭 CANAL MUNDO STREAMING PERÚ 🇵🇪","TU MARKETPLACE"]:
                    ...
                    for message_spam in messages_list:
                        ...
                        try:
                            ...
                            await client.send_message("@SpamHB15", f'<b>Error enviando mensajes a {i["group_id"]}</b> - <code>{i["group_name"]}<code>\nCausa:{error}',parse_mode="HTML")
                            resultado = False
                        if resultado:
                            await client.send_message("@SpamHB15", f'<b>Mensaje enviado a {i["group_id"]}</b> - <code>{i["group_name"]}</code>',parse_mode="HTML")  
                        await asyncio.sleep(10)
                        if j==2: break
            await client.send_message("@SpamHB15", f'<b>RONDA ACABADA</b>', parse_mode="HTML")
